 ...
 *  默认情况下 在debug环境中,会输出Debug及以上等级的log,在release环境中,会输出Info及以上等级的log
 *  
 *  @note ACLog 对向Xcode控制台输出的日志额外做了如下优化:
 *      ~> 会显示本次打印log操作所在的线程信息(优先级为"main" -> 自定义的GCDQueueLabel -> ThreadName -> ThreadID)
 *      ~> 如果你安装了XcodeColor这个Xcode插件(@see https://github.com/robbiehanson/XcodeColors ),ACLog会对输出的log按等级进行染色
 *
 ...
 *  @discussion 此宏建议配合和宏#ifdef DEBUG #endif配合,仅在调试时使用,避免在发布环境中输出无用日志
 *  例子如下
 @code
 //in .m file
 #ifdef DEBUG
 ...
 *      除了ACLogLevelError等级的log一定不会是异步输出,其他等级的log都会根据此值来决定是否采用异步日志输出
 *
